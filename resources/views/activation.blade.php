    ...
    <div class="ec-content-wrapper">
        <div class="content">
            <div class="breadcrumb-wrapper breadcrumb-contacts">
                ...
                <div>
                    <form action="{{ route('activite.delete.all') }}" method="post">
                        @csrf
                        @method('delete')
                        <button type="submit" class="btn btn-outline-danger" onclick="return confirm('Vous êtes sûr?');">
                        ...
                        @foreach ($Activitedate as $activitedat)
                            <div class="media pb-3 align-items-center justify-content-between">
                                ...
                                <span class=" font-size-12 d-inline-block"><i
                                        class="mdi mdi-clock-outline"></i>{{ $activitedat->created_at }}</span>
                            </div>
                        @endforeach
                    @elseif(isset($activites))
                        @foreach ($activites as $activite)
                            <div class="media pb-3 align-items-center justify-content-between">
                                ...
                                <div class="media-body pr-3 ">
                                    <a class="mt-0 mb-1 font-size-15 text-dark" href="#"><b>{{ $activite->name }}</b>
                                        {{ $activite->typeActivity }}</a>
                                    <a class="mt-0 mb-1 font-size-15 text-dark"
                                        href="#">{{ $activite->description }}</a>

                                </div>
                                <span class=" font-size-12 d-inline-block"><i
                                        class="mdi mdi-clock-outline"></i>{{ $activite->created_at }}</span>
                            </div>
                        @endforeach
                    @endif
                </div>
